...
async function getOromedWeather(day) {
    try {
        return fetch('http://127.0.0.1:6996/get_weather', {
            method: 'POST',
            headers: {
            ...
            .then(data => {
                if (data.error) {
                    ...
                    console.log(data)
                    processWeather(data.res[0], 365 - day)
                }
            });
    } catch (error) {
        console.log("Failed to get weather on day: ", day, "error: ", error)
    }
}
...
function processWeather(weather, day) {
    ...
    if (day == 0) { // varables only set for current day (eg wind speed, sunset/rise time)
